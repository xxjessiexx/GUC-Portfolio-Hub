...
export default function AdminFlaggedProjects() {
  ...
  const filtered = useMemo(
    () =>
      allFlaggedProjects.filter(
        (project) => {
          const haystack =
            `${project.title} ${project.student} ${getDisplayCourse(
              project
            )} ${project.reason}`.toLowerCase();
    ...
    if (decision.type === "project") {
      ...
      setToastData({
        ...
        description:
          decision.nextStatus ===
          "accepted"
            ? "The student appeal was accepted successfully."
            : "The student appeal was rejected successfully.",
        type: "success",
      });
    }
